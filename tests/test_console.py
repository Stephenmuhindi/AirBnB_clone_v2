...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            cons.onecmd('create City name="Kiambu"')
            mdl_id = cout.getvalue().strip()
            clear_stream(cout)
            self.assertIn('City.{}'.format(mdl_id), storage.all().keys())
            cons.onecmd('show City {}'.format(mdl_id))
            self.assertIn("'name': 'Kiambu'", cout.getvalue().strip())
            clear_stream(cout)
            cons.onecmd('create User name="Maureen" age=42 height=5')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'Maureen'", cout.getvalue().strip())
            self.assertIn("'age': 42", cout.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            self.assertIn('kakazablone@gmail.com', result)
            self.assertIn('031', result)
            cursor.close()
            dbc.close()
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="kakazablone@gmail.com", password="031")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('kakazablone@gmail.com', result)
            self.assertIn('031', result)
            self.assertIn('kakazablone@gmail.com', cout.getvalue())
            self.assertIn('031', cout.getvalue())
            cursor.close()
            dbc.close()
    ...
    def test_db_count(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            cons.onecmd('create State name="Githurai"')
            clear_stream(cout)
